...
public class OrderManagement extends JFrame {
	...
	public OrderManagement() {
		...
		btnNewButton.setBounds(174, 20, 98, 30);
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					boolean exists = psi.checkOrderExistsById(ID);
					if(exists == true)
					{
					...
					{
						JOptionPane.showMessageDialog(null,"沒有找到該筆訂單", "修改失敗", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
		...
		btnNewButton_2.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					if(exists == true)
					{
						int orderID = Integer.parseInt(orders_id.getText());
						ProductOrders productorders = new ProductOrders();
					...
					{
						JOptionPane.showMessageDialog(null,"沒有找到該筆訂單", "刪除失敗", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(null, "請輸入有效的數字！", "輸入錯誤", JOptionPane.ERROR_MESSAGE);
		...
		btnNewButton_2.setBounds(298, 20, 98, 30);
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				ProductOrderManager pod = new ProductOrderManager();
				pod.setVisible(true);
				dispose();
			}
		});
		btnNewButton_1.setFont(new Font("微軟正黑體", Font.BOLD, 14));
		btnNewButton_1.setBounds(268, 257, 98, 30);
		contentPane.add(btnNewButton_1);
		
		JButton btnNewButton_1_1 = new JButton("查看全部訂單");
		...
		btnNewButton_1_1.setBounds(61, 257, 133, 30);
